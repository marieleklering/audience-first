...
Not all feedback is equal. After running two personas, sort the output into four
categories before making any changes.

| Category | What to do |
|---|---|
| Universal (both flagged) | Fix first |
| Audience-specific (one flagged) | Decide based on primary reader |
...
**Stylistic** feedback is opinion. The persona has a perspective, not authority.
Act on it only if it matches your own read of the document.

**Surprising** feedback is the most valuable. If a persona flags something you did
not expect to be a problem, that is a blind spot. Prioritise it.

## Personas Are Inputs, Not Editors
...
on what you judge to be correct.
...
When feedback feels too easy, increase friction by adding prior failure or explicit
suspicion to the persona definition. See `docs/building-personas.md` for how.
